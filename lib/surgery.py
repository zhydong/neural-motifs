...
def filter_dets(boxes, obj_scores, obj_classes, rel_inds, pred_scores):
    """Algo:
    1)relation score = subj_score * pred_score * obj_score
    2)sort the relation score across relations by descending order
    3)re-order the pred_score and rel_inds by the order of 2)
    Args:
    ...
    # embed(header='filter det in surgery.py')
